...
class AbstractImageFilledShapes:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE",),
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff, "display": "number", "description": "Seed for shapes randomness."}),
                "randomize": ("BOOLEAN", {"default": False, "label": "Enable Randomization", "description": "If true, randomize shapes per layer, colors, size, position, and blend mode using the seed."}),
                "num_layers": ("INT", {"default": 1, "min": 1, "max": 10, "step": 1, "display": "number", "description": "Number of shape layers to add."}),
                "shapes_per_layer": ("INT", {"default": 10, "min": 1, "max": 100, "step": 1, "display": "number", "description": "Number of shapes to draw per layer."}),
                "max_shape_size": ("INT", {"default": 256, "min": 10, "max": 1024, "step": 1, "display": "number", "description": "Maximum size (width or height) of a single shape."}),
                "filled_shape_color_mode": (["random", "hex"] + [c.lower() for c in ["Red", "Green", "Blue", "Cyan", "Magenta", "Yellow", "Black", "White"]], {"default": "random", "description": "Color selection mode for filled shapes. 'random' uses implicit RGB."}),
                "filled_shape_hex": ("STRING", {"default": "", "placeholder": "#RRGGBB", "description": "Hex code for filled shapes color (used if Filled Shape Color Mode is 'hex')."}),
                "alpha": ("INT", {"default": 255, "min": 0, "max": 255, "step": 1, "display": "number", "description": "Alpha (transparency) for shapes (0 is fully transparent, 255 is fully opaque)."}),
                "shape_position": (["full", "upper_right", "upper_left", "lower_right", "lower_left", "center"], {"default": "full", "description": "Restrict shapes to a specific position/quadrant."},),
                "blend_mode": (BLEND_MODES, {"default": "normal", "description": "Blending mode to composite the shapes layer onto the base image."}),
                "feather_layer": ("BOOLEAN", {"default": False, "label": "Feather Layer", "description": "Apply Gaussian blur to the alpha channel of the shapes layer."}),
