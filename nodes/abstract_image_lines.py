    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff, "display": "number", "tooltip": "Seed for lines/zigzags randomness."}),
                "randomize": ("BOOLEAN", {"default": False, "label": "Enable Randomization", "tooltip": "If true, randomize lines per layer, colors, width, position, and blend mode using the seed."}),
                "num_layers": ("INT", {"default": 1, "min": 1, "max": 10, "step": 1, "display": "number", "tooltip": "Number of line/zigzag layers to add."}),
                "lines_per_layer": ("INT", {"default": 10, "min": 1, "max": 100, "step": 1, "display": "number", "tooltip": "Number of lines/zigzags to draw per layer."}),
                "line_width_min": ("INT", {"default": 5, "min": 1, "max": 100, "step": 1, "display": "number", "tooltip": "Minimum width for lines and zig-zags."}),
                "line_width_max": ("INT", {"default": 30, "min": 1, "max": 100, "step": 1, "display": "number", "tooltip": "Maximum width for lines and zig-zags."}),
                "line_zigzag_color_mode": (["random", "hex"] + [c.lower() for c in ["Red", "Green", "Blue", "Cyan", "Magenta", "Yellow", "Black", "White"]], {"default": "random", "tooltip": "Color selection mode for lines and zig-zags. 'random' uses implicit RGB."}),
                "line_zigzag_hex": ("STRING", {"default": "", "placeholder": "#RRGGBB", "tooltip": "Hex code for line/zigzag color (used if Line/Zigzag Color Mode is 'hex')."}),
                "alpha": ("INT", {"default": 255, "min": 0, "max": 255, "step": 1, "display": "number", "tooltip": "Alpha (transparency) for lines/zigzags (0 is fully transparent, 255 is fully opaque)."}),
                "shape_position": (["full", "upper_right", "upper_left", "lower_right", "lower_left", "center"], {"default": "full", "tooltip": "Restrict lines/zigzags to a specific position/quadrant."},),
                "blend_mode": (BLEND_MODES, {"default": "normal", "tooltip": "Blending mode to composite the lines/zigzags layer onto the base image."}),
                "feather_layer": ("BOOLEAN", {"default": False, "label": "Feather Layer", "tooltip": "Apply Gaussian blur to the alpha channel of the lines/zigzags layer."}),
